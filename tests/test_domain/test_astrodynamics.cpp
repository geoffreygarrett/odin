...
constexpr SCALAR mu         = 398'600.4418;// (from Vallado) This is Earth's gravitational constant in km^3/s^2
...
TEST(rv2coeTest, Case1) {
    ...
    auto [p, a, e, i, Omega, omega, nu, dummy1, dummy2, dummy3] = rv2coe(r, v, mu);
...
TEST(coe2rvTest, Case1) {
    ...
    auto [r, v] = coe2rv(p, a, e, i * DEG_TO_RAD, Omega * DEG_TO_RAD, omega * DEG_TO_RAD, nu * DEG_TO_RAD, mu);

    // Compare the result to the expected values, converted to the same units
    const auto r_expected = Vector3<SCALAR>(6'524.834, 6'862.875, 6'448.296); // Position vector in km
    const auto v_expected = Vector3<SCALAR>(4.901'327, 5.533'756, -1.976'341);// Velocity vector in km/s
...
TEST(RoundTripTest, Case1) {
    ...
    auto [p, a, e, i, Omega, omega, nu, dummy1, dummy2, dummy3] = rv2coe(r_orig, v_orig, mu);

    // Now convert back to position and velocity
    auto [r, v] = coe2rv(p, a, e, i, Omega, omega, nu, mu);
    ...
    auto [p_rt, a_rt, e_rt, i_rt, Omega_rt, omega_rt, nu_rt, dummy1_rt, dummy2_rt, dummy3_rt] = rv2coe(r, v, mu);

    // Check that the original and round-trip orbital elements match, within some tolerance
    EXPECT_NEAR(a, a_rt, TOL);
