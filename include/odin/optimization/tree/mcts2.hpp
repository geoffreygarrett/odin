...
struct SelectionPolicy {
    Node *operator()(Node *node) {
        std::cout << "Selecting child from " << node->children.size() << " children.\n";
        std::cout << "Node reward: " << node->reward << "\n";
        std::cout << "Node reward: " << node->state << "\n";
        ...
        std::cout << "Selected child: " << (*selected_child)->state << "\n";
...
struct UBC1 : public SelectionPolicy<UBC1<Node, Float>, Node, Float> {
    ...
    Float value(Node *child, Node *parent) {
        if (!child || !parent) {
            std::cout << "Child or parent node is null.\n";
            return std::numeric_limits<Float>::quiet_NaN();// Or some other safe value.
        }
        Float value = child->reward + cp_ * std::sqrt(std::log(parent->visit_count) / child->visit_count);
        std::cout << "Calculated value: " << value << "\n";
        return value;
    }
};
...
public:
    ...
    node_type *select_node(node_type *node) {
        std::cout << "Selecting child from node with " << node->children.size() << " children.\n"
                  << std::endl;
        if (node == nullptr) {
            throw std::runtime_error("Null node in select_node");
        }

        try {
            node_type *selected = mfn_selection_policy(node);
            if (selected) {
            ...
            std::cout << "MEOOO " << std::endl;
            if (selected) {
                std::cout << "Selected pointer: " << selected << "\n";
                std::cout << "Selected child from node with " << selected->state << " state.\n";
                std::cout << "Selected pointer after: " << selected << "\n";
            } else {
                std::cout << "No selected child to print state.\n";
            }
            return selected;
    ...
    void expand_all_actions(node_type *node) {
        try {
            ...
            if (mfn_is_terminal(node->state)) {
                ...
                std::cout << "Node is terminal, stopping expansion.\n";
                return;
            }

            // Generate possible actions
            auto possible_actions = mfn_get_actions(node->state);
            std::cout << "Generated " << possible_actions.size() << " possible actions.\n";
            ...
            for (auto const &action: possible_actions) {
                ...
                std::cout << "Generated new state from action.\n";

                // Check for duplicate nodes
                if (std::none_of(node->children.begin(), node->children.end(), [&](auto &child) { return child && child->state == new_state; })) {
                    ...
                    std::cout << "Created new child node. Total children: " << node->children.size() << ".\n";
                }
            }
            ...
            if (node->children.empty()) {
                node->is_terminal = true;
                std::cout << "No new children created, node marked as terminal.\n";
            }
        } catch (const std::exception &e) {
...
//        }
        ...
        while (iterations--) {
            ...
            while (!node->is_leaf()) {
                ODIN_VLOG(40) << "Selecting from node: " << node->state;
                node = select_node(node);
                std::cout << "Selected node: " << node->state << std::endl;
