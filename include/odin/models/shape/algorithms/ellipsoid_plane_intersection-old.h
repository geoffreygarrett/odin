...
template<typename U>
std::tuple<U, U, Vector<U, 3>> compute_ellipse_properties(U A, U B, U C, U D,
                                                          U E, U G, U h,
                                                          Vector<U, 3> n_pqr) {
...
std::tuple<U, U, U, U, U, U, U, U, U, U, U, U, U, U, U, U, U, U,
           std::array<std::array<U, 3>, 3>, Eigen::Matrix3<U>>
ellipse_of_intersection(U l, U m, U n, U f, U a, U b, U c) {
    ...
    Vector<U, 3> normal{l, m, n};
    U            k = normal.norm();

    // Establish semi-axes of ellipsoid
    Vector<U, 3> s_xyz{a, b, c};

    // Normalize normal vector, given sign of plane offset (Section 2.2, Eq. 44)
    Vector<U, 3> n_xyz = (f >= 0) ? normal / k : normal / -k;
    U            h     = (f >= 0) ? f / k : f / -k;

    // Ensure that v (component 3) is the largest component of the normal vector
    auto max_idx = n_xyz.argsort()[2];                 // (Section 2.2, Eq. 44)
    auto indices = std::array<std::size_t, 3>{0, 1, 2};// indices of {0, 1, 2}
    std::swap(indices[2], indices[max_idx]);           // swap 2 and max_idx
    auto P = permutation_matrix<U, 3>(indices);// frame transformation matrix
    ...
    Vector<U, 3> n_pqr(
            {n_xyz[indices[0]], n_xyz[indices[1]], n_xyz[indices[2]]});
    Vector<U, 3> s_pqr(
            {s_xyz[indices[0]], s_xyz[indices[1]], s_xyz[indices[2]]});
    ...
    // construct the ζ-axis, which is orthogonal to the plane's normal vector
    Eigen::Vector3<U> zeta_axis = Eigen::Vector3<U>::UnitZ().cross(n_xyz);
    zeta_axis.normalize();

    // construct the η-axis, which is orthogonal to the zeta axis and the plane's normal vector
    Eigen::Vector3<U> eta_axis = n_xyz.cross(zeta_axis);
    eta_axis.normalize();
