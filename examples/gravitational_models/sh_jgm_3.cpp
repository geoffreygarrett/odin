...
SphericalHarmonics<Scalar, Degree, Order> create_spherical_harmonics_model(Scalar mu, Scalar radius) {
    ...
    //    CoefficientTransformations<Scalar, Degree, Order>::denormalize_in_place(
    //            Cnm_values,
    //            Snm_values);
...
int main() {
    ...
    const double mu     = 398600.4418 * 1e9;
    const double radius = 6378136.3;

    const double step_size  = radius / 100;
    const double grid_range = radius * 2;
    ...
    auto spherical_harmonic = create_spherical_harmonics_model(mu, radius);

    tbb::concurrent_vector<GridPoint> grid_points_sh_jgm_3;
    grid_points_sh_jgm_3.reserve(num_steps * num_steps);
    ...
    ODIN_LOG_INFO << "Computed the potential field on a grid of " << grid_points_sh_jgm_3.size() << " points";
...
#define OMIT_REGION 0.0
            ...
            [&](const tbb::blocked_range2d<int> &r) {
                ...
                for (int i = r.rows().begin(); i != r.rows().end(); ++i) {
                    for (int j = r.cols().begin(); j != r.cols().end(); ++j) {
                        ...
                        double z = 0.0;
                        //                        if (x * x + y * y + z * z > OMIT_REGION * OMIT_REGION) {
                        ...
                        grid_points_sh_jgm_3.emplace_back(GridPoint{position, potential, acceleration});
                        //                        }
                    }
                }
            });
    ...
    save_to_hd5(grid_points_sh_jgm_3, "sh_jgm_3.h5", radius, radius, radius, 0, mu, step_size, grid_range);


    tbb::concurrent_vector<GridPoint> grid_points_point_mass;
    grid_points_point_mass.reserve(num_steps * num_steps);

    HomogeneousSphere<Scalar> spherical(mu, radius);
    ...
    // translate a,b,c to the average, so this is spherical
    save_to_hd5(grid_points_point_mass, "point_mass.h5", radius, radius, radius, 0, mu, step_size, grid_range);
